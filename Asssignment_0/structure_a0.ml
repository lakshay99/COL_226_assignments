open Signature_a0
	...
	let rec print_abs_val abs_val = match abs_val with
		...
		if (eq_rev_absolute num2 []) then raise(DivisionByZero)
			...
			if ((List.length num1_modified) <= (num_1_take_digits)) then (div_num_helper num1_modified num2_modified 9)
				...
				if (lt_rev_absolute (List.rev num1_modified) (List.rev num2_modified)) then ([], num1_modified) 
					...
					([], _) ->	(div_num num1_modified num2_modified (num_1_take_digits+1))
				|	(quotient, remainder) ->if (eq_rev_absolute quotient []) then  (div_num num1_modified num2_modified (num_1_take_digits+1)) 
											else
												...
												|	_  -> let (temp1, temp2) = (div_num (List.append remainder (drop num_1_take_digits num1_modified)) num2_modified 0) in
															let new_quotient = (List.append quotient) in
	...
	let print_num big_int = 
		let (sign, abs_val) = big_int in
		if sign = Neg then "-" ^ (print_abs_val abs_val)
		else (print_abs_val abs_val);;
